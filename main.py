...
def detect_noise(img):
    ...
    extreme = np.sum((flat < 10) | (flat > 245)) / len(flat)
    return "salt_pepper" if extreme > 0.02 else "gaussian"
...
show(img2, add_salt_pepper(img2, amount=0.06), "Attels 2 - Partika",     gauss_k=5, sigma=1.0, median_k=3)
show(img3, add_gaussian_noise(img3, sigma=20), "Attels 3 - Zemenes",      gauss_k=5, sigma=0.8, median_k=3)
show(img4, add_salt_pepper(img4, amount=0.08), "Attels 4 - Architektura", gauss_k=5, sigma=1.0, median_k=5)
